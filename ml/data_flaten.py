...
harvest_rows = (
    df.loc[df[target_crop] > 0, ['LAT', 'LON', 'YEAR', 'MONTH']]
    .rename(columns={'YEAR': 'YEAR_t', 'MONTH': 'MONTH_t'})
)
merged = df.merge(harvest_rows.drop_duplicates(), on=['LAT', 'LON'], how='inner')

merged['month_diff'] = (
    (merged['YEAR_t'] - merged['YEAR']) * 12 +
    (merged['MONTH_t'] - merged['MONTH'])
)
...
df1_agg = (
    df1.groupby(keys + ['month_diff'], as_index=False)[agg_cols]
       .mean()
)

wide = (
    df1_agg.set_index(keys + ['month_diff'])[agg_cols]
           .unstack('month_diff')
)
